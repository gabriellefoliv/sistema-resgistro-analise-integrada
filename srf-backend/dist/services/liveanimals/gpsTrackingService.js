...
class GpsTrackingService {
    ...
    async getAll(requesterId) {
        const results = await __1.prisma.gpsTracking.findMany({
            select: {
                id: true,
                liveAnimalId: true,
    ...
    async getFormOptions() {
        ...
        const trackingDevices = await __1.prisma.trackingDevice.findMany({
            select: { id: true, brand: true, serialNumber: true },
            orderBy: { brand: 'asc' }
        });
        const monitoringMethods = await __1.prisma.monitoringMethod.findMany({
    ...
    async create(data, requesterId) {
        return __1.prisma.$transaction(async (tx) => {
            ...
            if (existingResult)
                throw new Error('Não é possível criar um registro de rastreio GPS para um animal que já possui um registro.');
    ...
    async update(recordId, data, requesterId) {
        return __1.prisma.$transaction(async (tx) => {
            ...
            if (!existingResult)
                throw new Error('Registro de rastreio GPS não encontrado.');
            ...
            if (existingResult2)
                throw new Error('Não é possível atualizar um registro de rastreio GPS para um animal que já possui um registro.');
