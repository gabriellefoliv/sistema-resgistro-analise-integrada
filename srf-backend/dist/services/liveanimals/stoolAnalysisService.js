...
exports.StoolAnalysisService = void 0;
const __1 = require("../..");
...
class StoolAnalysisService {
    ...
    async getAll(requesterId) {
        const results = await __1.prisma.stoolAnalysis.findMany({
            select: {
                id: true,
                veterinarianVisit: {
                    select: {
    ...
    async getFormOptions() {
        const [veterinarianVisits, processingTechnologies] = await Promise.all([
            ...
            __1.prisma.processingTechnology.findMany({
    ...
    async create(data, requesterId) {
        return __1.prisma.$transaction(async (tx) => {
            ...
            if (existingResult)
                throw new Error('Não é possível criar uma análise de fezes para uma visita veterinária que já possui uma análise de fezes.');
    ...
    async update(recordId, data, requesterId) {
        return __1.prisma.$transaction(async (tx) => {
            ...
            if (!existingResult)
                throw new Error('Análise de fezes não encontrada.');
            ...
            if (existingResult2)
                throw new Error('Não é possível atualizar uma análise de fezes para uma visita veterinária que já possui uma análise de fezes.');
            // Atualiza
...
exports.StoolAnalysisService = StoolAnalysisService;
//# sourceMappingURL=stoolAnalysisService.js.map