...
exports.PhysicalExamService = void 0;
const __1 = require("../..");
...
class PhysicalExamService {
    ...
    async getAll(requesterId) {
        const exams = await __1.prisma.physicalExam.findMany({
            select: {
                id: true,
                veterinarianVisit: {
                    select: {
        ...
        const examsWithPermission = await Promise.all(exams.map(async (e) => {
    ...
    async getFormOptions() {
        const [veterinarianVisits, generalConditions, mucous, hydrations] = await Promise.all([
            ...
            __1.prisma.enumPhysicalExamGeneralCondition.findMany({
                select: {
                    id: true,
                    name: true
                },
            ...
            __1.prisma.enumMucous.findMany({
                select: {
                    id: true,
                    name: true
                },
            ...
            __1.prisma.enumHydration.findMany({
                select: {
                    id: true,
                    name: true
                },
    ...
    async create(data, requesterId) {
        return __1.prisma.$transaction(async (tx) => {
            ...
            if (existingExam)
                throw new Error('Não é possível criar um exame físico para uma visita veterinária que já possui um exame físico.');
            // Cria o exame físico
    ...
    async update(recordId, data, requesterId) {
        return __1.prisma.$transaction(async (tx) => {
            ...
            if (!existingExam)
                throw new Error('Exame físico não encontrado.');
            ...
            if (existingExamForVisit)
                throw new Error('Não é possível atualizar um exame físico para uma visita veterinária que já possui um exame físico.');
...
exports.PhysicalExamService = PhysicalExamService;
//# sourceMappingURL=physicalExamService.js.map