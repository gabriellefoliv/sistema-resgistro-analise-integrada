...
class VaccineService {
    ...
    formId = 'vacina';
    async getAll(userId) {
        const vaccines = await __1.prisma.vaccineApplication.findMany({
            select: {
                id: true,
        ...
        const vaccinesWithPermission = await Promise.all(vaccines.map(async (v) => {
    ...
    async getFormOptions() {
        const [liveAnimals, veterinarianVisits, vaccines, vaccineTypes] = await Promise.all([
            ...
            __1.prisma.enumVaccineType.findMany({
                select: {
                    id: true,
                    name: true
                },
    ...
    async create(data, requesterId) {
        return __1.prisma.$transaction(async (tx) => {
            ...
            if (!existingVaccineType)
                throw new Error('Tipo de aplicação da vacina não encontrado.');
            ...
            if (existingVaccineApplication)
                throw new Error('Esta vacina já foi aplicada neste animal nesta data.');
    ...
    async update(recordId, data, requesterId) {
        return __1.prisma.$transaction(async (tx) => {
            ...
            if (!existingVaccineApplication)
                throw new Error('Aplicação da vacina não encontrada.');
            // Verifica se a visita veterinária existe (opcional)
            if (data.veterinarianVisitId) {
                const existingVisit = await tx.veterinarianVisit.findUnique({
            ...
            if (data.veterinarianVisitId) {
                ...
                if (existingVaccineApplication)
                    throw new Error('Aplicação da vacina já existe.');
            }
            else {
                const existingVaccineApplication = await tx.vaccineApplication.findFirst({
                    where: {
...
//# sourceMappingURL=vaccineService.js.map