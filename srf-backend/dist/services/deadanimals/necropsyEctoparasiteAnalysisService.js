...
exports.NecropsyEctoparasiteAnalysisService = void 0;
const __1 = require("../..");
const auditService_1 = require("../auditService");
class NecropsyEctoparasiteAnalysisService {
    auditService = new auditService_1.AuditService();
    formId = 'analiseectoparasitos-am';
    tableName = 'ectoparasiteAnalysisNecropsy';
    async getAll(requesterId) {
        const results = await __1.prisma.ectoparasiteAnalysisNecropsy.findMany({
            select: {
                id: true,
                necropsy: {
                    select: {
    ...
    async getFormOptions() {
        const [necropsies, genuses, species] = await Promise.all([
            ...
            __1.prisma.ectoparasiteGenus.findMany({
                select: { id: true, name: true },
                orderBy: { name: 'asc' }
            }),
            __1.prisma.ectoparasiteSpecie.findMany({
                select: { id: true, name: true },
                orderBy: { name: 'asc' }
            })
        ]);
    ...
    async update(recordId, data, requesterId) {
        return __1.prisma.$transaction(async (tx) => {
            ...
            if (!existingResult)
                throw new Error('Análise de ectoparasito não encontrada.');
            ...
            if (existingResultRepeat)
                throw new Error('Não é possivel atualizar para uma análise de ectoparasitos com informações repetidas (necropsia, gênero, espécie e subespécie).');
            // Atualiza
...
exports.NecropsyEctoparasiteAnalysisService = NecropsyEctoparasiteAnalysisService;
//# sourceMappingURL=necropsyEctoparasiteAnalysisService.js.map