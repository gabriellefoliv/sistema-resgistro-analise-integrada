...
exports.NecropsySampleService = void 0;
const __1 = require("../..");
const auditService_1 = require("../auditService");
class NecropsySampleService {
    auditService = new auditService_1.AuditService();
    formId = 'amostras-am';
    tableName = 'sampleAllocationNecropsy';
    async getAll(requesterId) {
        const samples = await __1.prisma.sampleAllocationNecropsy.findMany({
            select: {
                id: true,
                necropsy: {
                    select: {
    ...
    async getFormOptions() {
        const [necropsies, sampleTypes, status, storages] = await Promise.all([
            ...
            __1.prisma.necropsySampleType.findMany({
                select: { id: true, description: true },
                orderBy: { description: 'asc' }
            }),
            __1.prisma.enumNecropsySampleAllocationStatus.findMany({
    ...
    async create(data, requesterId) {
        return __1.prisma.$transaction(async (tx) => {
            ...
            if (existingSample)
                throw new Error('Não é possível criar amostras que compartilhem necrópsia e tipo.');
            ...
            if (data.sendSamples) {
                ...
                data.sendSamples.forEach(sendSample => {
                    ...
                    if (sendDate < necropsyDate)
                        throw new Error('A data de envio da amostra não pode ser anterior à data da necrópsia.');
                });
    ...
    async update(recordId, data, requesterId) {
        return __1.prisma.$transaction(async (tx) => {
            ...
            if (!existingSample)
                throw new Error('Amostra de necrópsia não encontrada.');
            // Verifica se a necropsia existe
            const existingNecropsy = await tx.necropsy.findUnique({
                where: {
                    id: data.necropsyId
            ...
            if (existingSampleWithSameData)
                throw new Error('Não foi possível atualizar a amostra de necrópsia, pois já existe uma amostra que compartilha a mesma necrópsia e tipo de amostra.');
...
exports.NecropsySampleService = NecropsySampleService;
//# sourceMappingURL=necropsySampleService.js.map