...
exports.NecropsyExamResultService = void 0;
const __1 = require("../..");
const auditService_1 = require("../auditService");
class NecropsyExamResultService {
    auditService = new auditService_1.AuditService();
    formId = 'resultadoexame-am';
    async getAll(userId) {
        const cpcrResults = await __1.prisma.cpcrResult.findMany({
            select: {
                id: true,
                performedDate: true,
                primer: true,
        ...
        const qpcrResults = await __1.prisma.qpcrResult.findMany({
            select: {
                id: true,
                performedDate: true,
                meanCt: true,
        ...
        const cpcrCreateLogs = await __1.prisma.changeLog.findMany({
            where: {
                table: 'cpcrResult',
        ...
        const qpcrCreateLogs = await __1.prisma.changeLog.findMany({
            where: {
                table: 'qpcrResult',
    ...
    async getCPCRFormOptions() {
        const [necropsies, sampleTypes, extractionTypes, targetGenes, suspiciousAgents, cpcrMethods, cpcrStatuses] = await Promise.all([
            ...
            __1.prisma.cpcrSampleType.findMany({
                select: { id: true, description: true },
                orderBy: { description: 'asc' }
            }),
            __1.prisma.extractionType.findMany({
                select: { id: true, name: true },
                orderBy: { name: 'asc' }
            }),
            __1.prisma.targetGene.findMany({
                select: { id: true, name: true },
                orderBy: { name: 'asc' }
            }),
            __1.prisma.suspiciousAgent.findMany({
                select: { id: true, name: true },
                orderBy: { name: 'asc' }
            }),
            __1.prisma.enumCpcrMethod.findMany({
                select: { id: true, name: true },
                orderBy: { name: 'asc' }
            }),
            __1.prisma.enumCpcrStatus.findMany({
                select: { id: true, name: true },
                orderBy: { name: 'asc' }
            })
        ]);
        return {
            ...
            sampleTypes: sampleTypes.map(s => ({ id: s.id, name: s.description || s.name })),
            extractionTypes,
            targetGenes,
            suspiciousAgents,
            cpcrMethods,
    ...
    async createCPCR(data, requesterId) {
    ...
    async deleteCPCR(recordId, requesterId) {
        return __1.prisma.$transaction(async (tx) => {
    ...
    async getQPCRFormOptions() {
        const [necropsies, sampleTypes, targetGenes, suspiciousAgents, qpcrStatuses] = await Promise.all([
            ...
            __1.prisma.qpcrSampleType.findMany({
                select: { id: true, description: true },
                orderBy: { description: 'asc' }
            }),
            __1.prisma.targetGene.findMany({
            ...
            __1.prisma.enumQpcrStatus.findMany({
                select: { id: true, name: true },
                orderBy: { name: 'asc' }
            })
        ]);
    ...
    async createQPCR(data, requesterId) {
        return __1.prisma.$transaction(async (tx) => {
            const existingNecropsy = await tx.necropsy.findUnique({ where: { id: data.necropsyId } });
            if (!existingNecropsy)
                throw new Error('Necrópsia não encontrada.');
    ...
    async deleteQPCR(recordId, requesterId) {
        return __1.prisma.$transaction(async (tx) => {
...
exports.NecropsyExamResultService = NecropsyExamResultService;
//# sourceMappingURL=necropsyExamResultService.js.map