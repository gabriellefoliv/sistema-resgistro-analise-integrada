...
exports.NecropsyService = void 0;
const __1 = require("../..");
const auditService_1 = require("../auditService");
class NecropsyService {
    auditService = new auditService_1.AuditService();
    formId = 'necropsia';
    tableName = 'necropsy';
    async getAll(userId) {
        const necropsies = await __1.prisma.necropsy.findMany({
            select: {
                id: true,
                deadAnimalId: true,
    ...
    async getFormOptions() {
        const [deadAnimals, identifiedGenders, bodyConditions, clinicalConditions, tutors, reproductiveConditions, ages, bodyMeasurementTypes] = await Promise.all([
            __1.prisma.deadAnimal.findMany({
            ...
            __1.prisma.enumBodyCondition.findMany({
                select: { id: true, name: true },
                orderBy: { name: 'asc' }
            }),
            __1.prisma.enumClinicalCondition.findMany({
                select: { id: true, name: true },
                orderBy: { name: 'asc' }
            }),
            ...
            __1.prisma.enumReproductiveCondition.findMany({
                select: { id: true, name: true },
                orderBy: { name: 'asc' }
            }),
            __1.prisma.enumAge.findMany({
                select: { id: true, name: true },
                orderBy: { name: 'asc' }
            }),
            __1.prisma.bodyMeasurementTypeNecropsy.findMany({
    ...
    async create(data, requesterId) {
        return __1.prisma.$transaction(async (tx) => {
            ...
            if (existingNecropsy)
                throw new Error('Já existe uma necrópsia para este animal.');
            ...
            if (data.bodyMeasurements) {
                data.bodyMeasurements.forEach(e => {
                    ...
                    if (countTypeId > 1)
                        throw new Error('Não é possível criar uma necrópsia com o tipo de medida corporal duplicado.');
                });
            }
            // Cria a necrópsia
        ...
        return __1.prisma.$transaction(async (tx) => {
            ...
            if (data.bodyMeasurements) {
                data.bodyMeasurements.forEach(e => {
                    const countTypeId = data.bodyMeasurements.filter(bm => bm.bodyMeasurementTypeId === e.bodyMeasurementTypeId).length;
                    if (countTypeId > 1)
                        throw new Error('Não é possível alterar uma necrópsia com o tipo de medida corporal duplicado.');
                });
            }
...
exports.NecropsyService = NecropsyService;
//# sourceMappingURL=necropsyService.js.map