...
import { deleteHelminthAnalysis } from "../../../services/deadanimals/helminthAnalysisService";

interface DeleteHelminthAnalysisModalProps {
    helminthAnalysis: GetAllHelminthAnalysisOutput;
    close: () => void;
    refresh: () => void;
}

export function DeleteHelminthAnalysisModal({ helminthAnalysis, close, refresh }: DeleteHelminthAnalysisModalProps) {
    ...
    async function handleDelete() {
        ...
        setError('');
        try {
            await deleteHelminthAnalysis(helminthAnalysis.id);
            refresh();
            close();
        } catch (error: any) {
            setError(error.response?.data?.error || 'Erro ao excluir análise de helminto.');
            setLoading(false);
        }
    }
            ...
            <div onMouseDown={close} className="modal-overlay flex justify-center items-center fixed top-0 left-0 w-full h-full bg-black/50 z-100 overflow-y-auto p-4">
                <div onMouseDown={(e) => e.stopPropagation()} className="modal relative flex flex-col bg-white justify-center items-center rounded-2xl shadow-xl px-10 pt-12 pb-6 gap-5 w-120">
                    ...
                    <p className="text-center font-bold text-lg">
                        Tem certeza que deseja EXCLUIR a análise de helminto do animal <span className="text-standard-blue">{helminthAnalysis.deadAnimalCode}</span>?
                    </p>
