...
export function DeleteEggCystAnalysisModal({ eggCystAnalysis, close, refresh }: DeleteEggCystAnalysisModalProps) {
    ...
    async function handleDelete() {
        ...
        try {
            ...
            setError(error.response?.data?.error || 'Erro ao excluir análise de ovos/cistos.');
            setLoading(false);
        }
    }
            ...
            <div onMouseDown={close} className="modal-overlay flex justify-center items-center fixed top-0 left-0 w-full h-full bg-black/50 z-100 overflow-y-auto p-4">
                <div onMouseDown={(e) => e.stopPropagation()} className="modal relative flex flex-col bg-white justify-center items-center rounded-2xl shadow-xl px-10 pt-12 pb-6 gap-5 w-120">
                    ...
                    <p className="text-center font-bold text-lg">
                        Tem certeza que deseja EXCLUIR a análise de ovos/cistos da espécie <span className="text-standard-blue">{eggCystAnalysis.eggCystSpecieName}</span> do animal <span className="text-standard-blue">{eggCystAnalysis.liveAnimalName}</span>?
                    </p>
